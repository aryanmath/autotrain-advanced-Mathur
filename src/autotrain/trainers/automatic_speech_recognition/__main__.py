...
def train(config):
    ...
    processor = Wav2Vec2Processor.from_pretrained(config.model, token=config.token)
    ...
    train_data = AutomaticSpeechRecognitionDataset(data=train_data, processor=processor, config=config)
    if config.valid_split is not None:
        valid_data = AutomaticSpeechRecognitionDataset(data=valid_data, processor=processor, config=config)
